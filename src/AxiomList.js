   ...
   const keyPress = (e)=>{
     ...
     const go = (subject,target,name) =>{
        if(subject && target && name){
            dispatch({ action: 'add' , for:type, payload:{name:name,subject:subject,target:target}, class:"axiom"})
            setName("")
            setSubject("")
            setTarget("")
           } 
        ...
        <div style={{height:"100vh",width:"200px",color:"white",padding:"30px;"}}>
            <div> 
                ...
                <FormControl variant="outlined">
                    ...
                    >
                        ...
                        {content.map(x=>{
                            return(<MenuItem value={x.uuid}>{globalState.getDisplayName(globalState,x)}</MenuItem>)
                        })}
                    </Select>
                </FormControl>
            ...
            {content && content.map(x=> {
                ...
                <div style={{margin:"10px"}} onClick={()=>{handleRowClick(x)}}>
                    <button onClick={()=>dispatch({ action: 'remove' , for:type, payload:x.uuid, class:"axiom"})}>X</button>
